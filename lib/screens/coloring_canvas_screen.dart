...
import 'package:color_world/managers/billing_manager.dart'; // Gerçek BillingManager import edildi
...
class _ColoringCanvasScreenState extends State<ColoringCanvasScreen> with WidgetsBindingObserver {
  ...
  Future<void> _initCanvas() async {
    _isAdFree = await BillingManager.isAdFree(); // Gerçek BillingManager'a yönlendirildi
    if (!_isAdFree) {
      _startAdTimer();
    }
    await _loadTemplate();
  ...
  Future<void> _startAdTimer() async {
    ...
    _adTimer = Timer.periodic(const Duration(seconds: 180), (timer) async {
      final adFree = await BillingManager.isAdFree(); // Gerçek BillingManager'a yönlendirildi
      if (adFree) {
        _isAdFree = true;
        _adTimer?.cancel();
        return;
  ...
  Future<void> _exportToGallery() async {
    ...
    try {
      ...
      if (byteData == null) throw Exception('Unable to capture image bytes.');

      final directory = await getApplicationDocumentsDirectory();
      final file = File('${directory.path}/color_world_${DateTime.now().millisecondsSinceEpoch}.png');
      await file.writeAsBytes(byteData.buffer.asUint8List());
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('${L.imageSaved}: ${file.path}')),
        );
      }
    } catch (e) {
